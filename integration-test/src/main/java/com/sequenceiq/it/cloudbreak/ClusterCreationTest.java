...
public class ClusterCreationTest extends AbstractCloudbreakIntegrationTest {
    ...
    @Parameters({"clusterName", "emailNeeded", "enableSecurity", "kerberosMasterKey", "kerberosAdmin",
            "kerberosPassword", "runRecipesOnHosts", "checkAmbari", "withRDSConfig", "autoRecoveryMode", "withGcsFs"})
    public void testClusterCreation(@Optional("it-cluster") String clusterName, @Optional("false") boolean emailNeeded,
            ...
            @Optional ("false") boolean withGcsFs) throws Exception {
        // GIVEN
        IntegrationTestContext itContext = getItContext();
        ...
        if (Boolean.TRUE.equals(withGcsFs)) {
            clusterRequest = setGcsFs(itContext, clusterRequest);
        }
    ...
    private ClusterRequest setGcsFs(IntegrationTestContext itContext, ClusterRequest clusterRequest) {
        Assert.assertNotNull(itContext.getContextParam(CloudbreakITContextConstants.GCSFSREQUEST,FileSystemRequest.class), "GCS filesystem was not configured");
        FileSystemRequest fsRequest = itContext.getContextParam(CloudbreakITContextConstants.GCSFSREQUEST, FileSystemRequest.class);
